...
import ru.vsu.cs.course1.graph.demo.Pair;
...
public class GraphAlgorithms {
    ...
    public static List<Pair<Integer, Integer>> fingAllContaisEdjes(Graph graph, int from, int to){

        class Inner{
            List<Pair<Integer, Integer>> startSearch() {
                List<List<Integer>> allPathes = new ArrayList<>();
                boolean[] visited = new boolean[graph.vertexCount()];
                recursionPath(from, to, visited, new ArrayList<>(), allPathes);
                return getEdjes(allPathes);
            }

            List<Integer> getMinPath(List<List<Integer>> allPathes){
                List<Integer> minSizePath = allPathes.get(0);
                int min = allPathes.get(0).size();
            ...
            List<Pair<Integer, Integer>> getEdjes(List<List<Integer>> allPathes){
                List<Integer> minSizePath = getMinPath(allPathes);
                List<Pair<Integer, Integer>> edjes = new ArrayList<>();
                for (int i = 0; i < minSizePath.size()-1; i++) {
                    boolean flag = true;
                    Pair<Integer, Integer> edje = new Pair<>(minSizePath.get(i), minSizePath.get(i+1));
                    for (List<Integer> l: allPathes) {
                        if(!containsPair(edje, l)){
                            flag = false;
                            break;
                        }
                    }
            ...
            boolean containsPair(Pair<Integer, Integer> edje, List<Integer> path){
                for (int i = 0; i < path.size()-1; i++) {
                    if(edje.getKey() == path.get(i) && edje.getValue() == path.get(i+1))
                        return true;
                }
                return false;
            }
            void recursionPath(int u, int to, boolean[] visited, List<Integer> currentPath, List<List<Integer>> paths) {
                visited[u] = true;
                currentPath.add(u);
                ...
                if (u == to) {
                    ...
                    for (int v : graph.adjacencies(u)) {
                        if (!visited[v]) {
                            recursionPath(v, to, visited, currentPath, paths);
                        }
                    }
                }
